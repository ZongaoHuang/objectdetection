...
class PP_YOLOE():
    def __init__(self, prob_threshold=0.8):
        ...
        self.session = onnxruntime.InferenceSession('models/mot_ppyoloe_l_36e_pipeline.onnx', so)
        self.input_size = (
...
class KeyPointDetector():
    def __init__(self):
        ...
        so = onnxruntime.SessionOptions()
        so.log_severity_level = 3
        self.session = onnxruntime.InferenceSession('models/dark_hrnet_w32_256x192.onnx', so)
    ...
    def visualize_pose(self, canvas, results):
        ...
        if kpt_nums == 17:  # plot models keypoint
